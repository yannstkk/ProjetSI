...
import { ArrowLeft, ArrowRight, Sparkles, Loader2, AlertTriangle, CheckCircle, XCircle, Info, RefreshCw } from "lucide-react";
...
export function Phase4C() {
    ...
    async function lancerAnalyse() {
        if (backlog.length === 0) {
            setIaError("Le backlog est vide — créez des User Stories en Phase 4 d'abord.");
            return;
        }
        ...
        try {
            const backlogTexte = backlog.map((us) =>
                [
                    `ID: ${us.id}`,
                    `Acteur: ${us.acteur || "(non défini)"}`,
                    `Je veux: ${us.veux || "(vide)"}`,
                    `Afin de: ${us.afin || "(vide)"}`,
                    `Priorité: ${us.priorite || "(non définie)"}`,
                    `Critères: ${us.criteres?.length ? us.criteres.join(" | ") : "(aucun)"}`,
                ].join("\n")
            ).join("\n\n---\n\n");

            const res = await authFetch("/api/mistral/analyser-backlog", {
                method: "POST",
                body: JSON.stringify({ contenu: backlogTexte }),
            });
    ...
    const stats = useMemo(() => {
        ...
        return {
            ...
            erreurs:        alertes.filter((a) => a.type === "erreur").length,
            avertissements: alertes.filter((a) => a.type === "avertissement").length,
            suggestions:    alertes.filter((a) => a.type === "suggestion").length,
        };
    }, [resultat]);
            ...
            <div className="max-w-5xl mx-auto space-y-6">
                ...
                {backlog.length === 0 && (
                    <div className="flex items-center gap-3 p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-800 text-sm">
                        ...
                        <span>
                            Le backlog est vide.{" "}
                            <Link to="/dashboard/phase4/backlog" className="underline font-medium">
                                Créez des User Stories
                            </Link>{" "}
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            ...
                            Analyse IA du backlog complet
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <p className="text-sm text-gray-600">
                            L'IA analyse l'ensemble de vos <strong>{backlog.length}</strong> User
                            {backlog.length > 1 ? " Stories" : " Story"} et détecte :
                        </p>
                        <ul className="text-sm text-gray-600 space-y-1 ml-4">
                            <li className="flex items-center gap-2">
                                <XCircle className="w-3.5 h-3.5 text-red-500 flex-shrink-0" />
                                Les US mal formulées ou incomplètes
                            </li>
                            <li className="flex items-center gap-2">
                                <AlertTriangle className="w-3.5 h-3.5 text-yellow-500 flex-shrink-0" />
                                Les doublons et redondances
                            </li>
                            <li className="flex items-center gap-2">
                                <AlertTriangle className="w-3.5 h-3.5 text-yellow-500 flex-shrink-0" />
                                Les incohérences entre US (conflits de priorité, contradictions)
                            </li>
                            <li className="flex items-center gap-2">
                                <Info className="w-3.5 h-3.5 text-blue-500 flex-shrink-0" />
                                Les suggestions d'amélioration (critères manquants, acteurs flous)
                            </li>
                        </ul>
                        ...
                        <div className="flex items-center gap-3 pt-2">
                            <button
                                ...
                                disabled={iaLoading || backlog.length === 0}
                                className="flex items-center gap-2 px-5 py-2.5 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-purple-300 transition-colors font-medium"
                            >
                                {iaLoading ? (
                            ...
                            {resultat && (
                                <button
                                    ...
                                    className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 border border-gray-200 px-3 py-2 rounded-lg hover:border-gray-300 transition-colors"
                                >
                        ...
                        {/* Indicateur résultat en cache */}
                        {resultat && !iaLoading && (
                            <p className="text-xs text-gray-400 flex items-center gap-1">
                                <CheckCircle className="w-3 h-3 text-gray-300" />
                                Résultats chargés depuis la session — relancez l'analyse si le backlog a changé.
                            </p>
                        )}
                    </CardContent>
                </Card>
                        ...
                        {stats.total === 0 && (
                            <Card>
                                <CardContent className="p-8 text-center">
                                    ...
                                    <p className="font-semibold text-gray-900 mb-1">Backlog validé</p>
                                    <p className="text-sm text-gray-500">
                                        Aucune incohérence détectée — votre backlog est prêt pour la Phase 5.
                                    </p>
                                </CardContent>
                            </Card>
                        )}
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {["tous", "erreur", "avertissement", "suggestion"].map((f) => (
                                        ...
                                        >
                                            ...
                                            : f === "erreur"       ? `Erreurs (${stats.erreurs})`
                                            : f === "avertissement"? `Avertissements (${stats.avertissements})`
                                                ...
                                                <CardContent className="p-4">
                                                    <div className="flex items-start gap-3">
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            ...
                                                            {alerte.usId && (
                                                                <Link
                                                                    to={`/dashboard/phase4/form?id=${alerte.usId}`}
                ...
                {resultat && stats?.erreurs > 0 && (
                    <p className="text-xs text-gray-400 text-right -mt-4">
                        Résolvez les erreurs avant de passer à la Phase 5.
                    </p>
                )}
