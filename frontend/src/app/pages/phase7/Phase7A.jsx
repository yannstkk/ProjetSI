...
import {
    Upload, FileText, X, ArrowRight, Database,
...
export function Phase7A() {
    ...
    async function analyserAvecIA() {
        ...
        setIaError("");
        try {
            const res = await authFetch("/api/mcd/analyser", {
                method: "POST",
                body: JSON.stringify({ contenuPlantuml: plantUmlCode }),
            });
    ...
    async function sauvegarderBdd() {
        if (!plantUmlCode.trim() || !nom.trim()) {
            setDbError("Importez un fichier et donnez un nom avant de sauvegarder.");
            return;
        }
        ...
        try {
            const res = await authFetch("/api/mcd", {
                method: "POST",
                body: JSON.stringify({
                    idProjet:        projet.id,
                    nom:             nom.trim(),
                    contenu:         plantUmlCode,
                    reponseMistral:  iaResult ? JSON.stringify(iaResult) : null,
                }),
            });
            if (!res.ok) throw new Error(`Erreur serveur ${res.status}`);
    ...
    function handleReset() {
        ...
        setMcdDbId(null); setIaResult(null); setIaError(""); setDbError("");
        setDiagramError(false);
        clearMCD();
    }
            ...
            <div className="max-w-5xl mx-auto space-y-6">
                ...
                {dbSuccess && (
                    <div className="flex items-center gap-2 p-3 bg-emerald-50 border border-emerald-200 rounded-lg text-emerald-700 text-sm">
                        ...
                        MCD sauvegardé en base de données avec succès.
                    </div>
                )}
                {dbError && (
                    <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">
                        <AlertTriangle className="w-4 h-4 flex-shrink-0"/> {dbError}
                    </div>
                )}
                ...
                {!hasDiagram && (
                    ...
                    >
                        <input ref={fileInputRef} type="file"
                            accept=".puml,.plantuml,.txt,.iuml" className="hidden"
                            onChange={handleFileChange}/>
                        <div className="w-16 h-16 bg-indigo-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                            <Upload className="w-8 h-8 text-indigo-600"/>
                        </div>
                        <h3 className="font-semibold text-gray-900 text-lg mb-2">
                            Importer votre MCD PlantUML
                        </h3>
                        ...
                        <div className="col-span-5 space-y-4">
                            ...
                            <div className="flex items-center gap-3 px-4 py-3 bg-emerald-50 border border-emerald-200 rounded-xl">
                                ...
                                <button onClick={() => fileInputRef.current?.click()}
                                    className="text-xs text-emerald-600 hover:text-emerald-700 underline flex-shrink-0">
                                    Changer
                                </button>
                                <input ref={fileInputRef} type="file"
                                    accept=".puml,.plantuml,.txt,.iuml" className="hidden"
                                ...
                                <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                                    ...
                                    {diagramError && (
                                        <span className="text-xs text-orange-600 flex items-center gap-1">
                                            <AlertTriangle className="w-3 h-3"/> Erreur de rendu
                                        </span>
                                    )}
                                    {mcdDbId && (
                                        <span className="text-xs text-emerald-600 flex items-center gap-1">
                                            <Database className="w-3 h-3"/> Sauvegardé (ID {mcdDbId})
                                        </span>
                                    )}
                                </div>
                                <div className="p-4 min-h-[320px] flex items-center justify-center bg-gray-50">
                                    {diagramUrl && !diagramError ? (
                                        <img src={diagramUrl} alt="MCD"
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={e => {
                                            ...
                                            saveMCD({ code: plantUmlCode, diagramUrl, fileName, nom: e.target.value, mcdDbId, iaResult });
                                        }}
                                        placeholder="Ex: MCD Facturation v1"
                                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all"
                                    />
                                </div>
                                <button onClick={sauvegarderBdd}
                                    disabled={dbLoading || !nom.trim()}
                                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:bg-emerald-300 transition-colors font-medium text-sm">
                                    {dbLoading
                                        ? <><Loader2 className="w-4 h-4 animate-spin"/> Sauvegarde...</>
                                        : <><Database className="w-4 h-4"/>
                                          {mcdDbId ? "Mettre à jour en BDD" : "Sauvegarder en BDD"}</>
                                    }
                                </button>
                            </div>

                            {/* Analyser avec IA */}
                            <button onClick={analyserAvecIA} disabled={iaLoading}
                                className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-purple-600 text-white rounded-xl hover:bg-purple-700 disabled:bg-purple-300 transition-colors font-medium">
                                {iaLoading
                            ...
                            {iaError && (
                                <p className="text-sm text-red-600 flex items-center gap-1">
                                    <AlertTriangle className="w-3.5 h-3.5"/> {iaError}
                                </p>
                            )}
                        </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {entites.map((e, i) => (
                            <span key={i} className="px-2.5 py-1 bg-blue-50 border border-blue-200 rounded-lg text-xs font-medium text-blue-700">
                                {e.nom}
                                {e.attributs?.length > 0 && (
                                    <span className="ml-1 text-blue-400">({e.attributs.length} attr.)</span>
                                )}
                            </span>
                        ))}
                    </div>
