...
import {
    loadClassification,
    saveClassification,
    importerDepuisPhase1,
    COLONNES,
} from "./components/helpers/classificationStorage";
...
function trouverColonne(classification, itemId) {
    return Object.keys(classification).find((col) =>
        classification[col].some((i) => i.id === itemId)
    );
}
...
export function Phase3C() {
    ...
    // Chargement initial : sessionStorage ou import depuis Phase 1
    ...
    function update(newClassification) {
    ...
    function handleDragOver({ active, over }) {
        ...
        const destCol   = Object.keys(COLONNES).includes(over.id)
            ? over.id
            : trouverColonne(classification, over.id);
    ...
    function handleDragEnd({ active, over }) {
        ...
        const sourceCol = trouverColonne(classification, active.id);
        const destCol   = trouverColonne(classification, over.id) ||
            (Object.keys(COLONNES).includes(over.id) ? over.id : null);
        ...
        if (oldIndex !== newIndex) {
            const newClass = { ...classification };
            newClass[sourceCol] = arrayMove(newClass[sourceCol], oldIndex, newIndex);
            update(newClass);
        }
    }
    ...
    function handleValider(itemId) {
        const col = trouverColonne(classification, itemId);
        if (!col) return;
        const newClass = { ...classification };
        newClass[col] = newClass[col].map((i) =>
            i.id === itemId ? { ...i, valide: !i.valide } : i
        );
        update(newClass);
    }
    ...
    function handleSupprimer(itemId) {
        ...
        if (!col) return;
        const newClass = { ...classification };
        newClass[col] = newClass[col].filter((i) => i.id !== itemId);
        update(newClass);
    }
    ...
    function handleEditer(itemId, newTexte) {
        ...
        const newClass = { ...classification };
        newClass[col] = newClass[col].map((i) =>
            i.id === itemId ? { ...i, texte: newTexte } : i
        );
        update(newClass);
    }
    ...
    function handleAjouter(colonneId, texte) {
        ...
        const newClass = { ...classification };
        newClass[colonneId] = [...newClass[colonneId], newItem];
        update(newClass);
    }

    // ── Réimport depuis Phase 1 ───────────────────────────────────────────────

    function handleReimporter() {
        const fresh = importerDepuisPhase1();
        update(fresh);
        setIaMessage("");
        setIaError("");
    }
    ...
    // ── Effacer marquages doublons ────────────────────────────────────────────

    function effacerDoublons() {
        const newClass = { ...classification };
        Object.keys(newClass).forEach((col) => {
            newClass[col] = newClass[col].map((i) => ({ ...i, doublonSuspecte: false }));
        });
        update(newClass);
        setIaMessage("");
    }
    ...
    const hasDoublons = Object.values(classification)
        .flat()
        .some((i) => i.doublonSuspecte);
                ...
                <div className="flex items-start justify-between">
                    <div>
                        ...
                        <p className="text-gray-600">
                            Phase 3 : Organiser et valider les éléments capturés
                        </p>
                    </div>

                    <div className="flex items-center gap-2">
                        <button
                            onClick={handleReimporter}
                        ...
                        <button
                            onClick={detecterDoublons}
                            disabled={iaLoading}
                            className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-purple-300 text-sm font-medium transition-colors"
                        >
                            {iaLoading ? (
                                <>
                    ...
                    <div className="grid grid-cols-4 gap-4">
                        {Object.keys(COLONNES).map((colonneId) => (
                            <ColonneKanban
                                key={colonneId}
                                colonneId={colonneId}
                                items={classification[colonneId] || []}
